...
void Plasma::Draw()
{
	...
	for (unsigned int x = 0; x < m_width; x++) {
		for (unsigned int y = 0; y < m_height; y++) {
			double n = (noise(double(x)/double(m_width), double(y)/double(m_height), sin(m_radians)) + 1.0) * 0.5; // scale noise() -1.0..1.0 to 0.0..1.0
	...
	m_radians += (M_PI*0.001);
	if (m_radians >= M_PI) m_radians -= M_PI;
}
