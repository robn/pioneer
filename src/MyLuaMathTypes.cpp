...
namespace MyLuaFixed {
	bool isFixed (lua_State *L, int index)
	{
		fixed *f;
		if (lua_type(L, index) != LUA_TUSERDATA) return NULL;
		f = (fixed *)luaL_checkudata(L, index, MYLUA_FIXED);
	...
	fixed *checkFixed (lua_State *L, int index)
	{
		...
		luaL_checktype(L, index, LUA_TUSERDATA);
		f = (fixed *)luaL_checkudata(L, index, MYLUA_FIXED);
		if (f == NULL) luaL_typerror(L, index, MYLUA_FIXED);
		return f;
	}

	fixed *pushFixed (lua_State *L)
	{
		fixed *f = (fixed *)lua_newuserdata(L, sizeof(fixed));
		luaL_getmetatable(L, MYLUA_FIXED);
		lua_setmetatable(L, -2);
		return f;
	}
	...
	// XXX add useful lua-side methods here

	static const luaL_reg Fixed_methods[] = {
		{ "new", Fixed_new },
		{ 0, 0 }
	};
