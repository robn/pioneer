...
void NotifyOfCrime(Ship *s, enum Crime crime)
{
	...
	if (station) {
		...
		Pi::game->GetGameLog()->AddMessageFrom(station->GetLabel(), stringf(Lang::X_CANNOT_BE_TOLERATED_HERE, formatarg("crime", crimeNames[crimeIdx])), GameLog::PRIORITY_IMPORTANT);
