	...
	for (Space::BodyNearIterator i = nearby.begin(); i != nearby.end(); ++i) {
		...
		switch ((*i)->GetType()) {
			...
			case Object::SHIP: {
				...
				if ((*i) == Pi::player->GetPlayerController()->GetCombatTarget()) c.isSpecial = true;
			...
			case Object::HYPERSPACECLOUD:
				...
				if ((*i) == Pi::player->GetPlayerController()->GetNavTarget()) c.isSpecial = true;
...
{
	if (!Pi::player->GetPlayerController()->GetCombatTarget()) {
	...
	LuaObject<Ship>::PushToLua(static_cast<Ship*>(Pi::player->GetPlayerController()->GetCombatTarget()));
	lua_call(l, 3, 1);
