...
void StationShipEquipmentForm::FitItem(Equip::Type t)
{
	...
	if (Pi::player->GetMoney() < m_station->GetPrice(t)) {
		Pi::game->GetViewManager()->GetShipCpanel()->MsgLog()->Message("", Lang::YOU_NOT_ENOUGH_MONEY);
		return;
	}

	if (!freespace || stats->free_capacity < Equip::types[t].mass) {
		Pi::game->GetViewManager()->GetShipCpanel()->MsgLog()->Message("", Lang::NO_SPACE_ON_SHIP);
		return;
	}
...
void StationShipEquipmentForm::FitItemForce(Equip::Type t, int pos) {
	...
	Pi::game->GetViewManager()->GetShipCpanel()->MsgLog()->Message("", Lang::FITTING+std::string(Equip::types[t].name));
...
void StationShipEquipmentForm::RemoveItemForce(Equip::Type t, int pos) {
	...
	Pi::game->GetViewManager()->GetShipCpanel()->MsgLog()->Message("", Lang::REMOVING+std::string(Equip::types[t].name));
