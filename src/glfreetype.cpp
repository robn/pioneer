...
TextureFontFace::TextureFontFace(const std::string &config_filename) : m_config(config_filename)
{
	...
	int err;
	m_pixSize = a_height;
	if (0 != (err = FT_New_Face(library, std::string(PIONEER_DATA_DIR "/fonts/" + filename_ttf).c_str(), 0, &m_face))) {
	...
	for (int chr=32; chr<127; chr++) {
		...
		err = FT_Load_Char(m_face, chr, FT_LOAD_DEFAULT);
		if (err) {
			fprintf(stderr, "Errod %d loading glyph\n", err);
			continue;
		}
