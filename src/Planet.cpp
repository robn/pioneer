...
void Planet::GetAtmosphericState(double dist, double *outPressure, double *outDensity) const
{
	...
	const double surfaceGravity_g = -G*sbody->GetMass()/pow((sbody->GetRadius()),2); // should be stored in sbody
	...
	const double surfaceTemperature_T0 = sbody->phys.averageTemp; //K
