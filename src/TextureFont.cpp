...
TextureFont::TextureFont(Graphics::Renderer *r, const FontConfig &fc, const vector2f &aspectScale) : Font(fc)
{
	int err; // used to store freetype error return codes

	const int a_width  = int(GetConfig().Int("PixelWidth")  / aspectScale.x);
	const int a_height = int(GetConfig().Int("PixelHeight") / aspectScale.y);
	...
	m_texture.Reset(r->CreateTexture(descriptor));
